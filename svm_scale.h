#ifndef SVM_SCALING_H_
#define SVM_SCALING_H_
...
void output_target(FILE *fp,double value)
{
	if(y_scaling)
	{
		if(value == y_min)
...
//int svm_sacle(double lower,double upper,char *filename,char *savefilename,char *restore_filename)
...
#define SKIP_TARGET { while(isspace(*p)) ++p;while(!isspace(*p)) ++p;}

#define SKIP_ELEMENT {while(*p!=':') ++p;++p;while(isspace(*p)) ++p;while(*p && !isspace(*p)) ++p;}
	...
	printf("5\n");
	/* pass 3: scale */
	filename=(char*)malloc(sizeof(char)*(strlen(argv[argc-1])+6));
	sprintf(filename,"%s.scale",argv[argc-1]);
